...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i += 1) {
    ...
    else if (value === '--drive') args.drive = true;
...
async function sampleCanvas(page) {
  const locator = page.locator('#game-canvas').first();
  const rect = await locator.boundingBox();
  if (!rect || rect.width < 32 || rect.height < 32) {
    return { ok: false, reason: 'canvas-too-small', rect };
  }

  const buffer = await page.screenshot({ clip: rect, timeout: 120000 });
...
async function main() {
  ...
  await page.waitForSelector('#game-canvas', { state: 'visible', timeout: 120_000 });
  await page.waitForFunction(() => (window.__THREE_GAME_DIAGNOSTICS__?.frame ?? 0) > 2, { timeout: 120000 });
  if (args.drive) {
    ...
    if (args.mobile) {
      ...
      await page.mouse.down(); await page.waitForTimeout(2500); await page.mouse.up();
    } else {
